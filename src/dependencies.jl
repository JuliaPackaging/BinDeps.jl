...
# same provider.
#
function issatisfied(deps::LibraryGroup)
    viable_providers = nothing
    for dep in deps.deps
        providers = map(x->x[1],_find_library(dep))
        if viable_providers == nothing
            viable_providers = providers
        else
            intersect!(viable_providers,providers)
        end
    end
    !isempty(viable_providers)
end

function satisfy!(dep::Union(LibraryDependency,LibraryGroup), methods = defaults)
    if !issatisfied(dep)
        if !applicable(dep)
            return
        end
        ...
        push!(ret.args,
            esc(quote
                    ...
                    if bindeps_context.do_install
                        for d in bindeps_context.deps
                            BinDeps.satisfy!(d)
                            lib = BinDeps._find_library(d)[1][2]
                            if lib != ""
...
macro load_dependencies(args...)
    ...
    for dep in context.deps
        ...
        push!(ret.args,quote
            const $(esc(s)) = BinDeps._find_library($dep)[1][2]
            if isempty($(esc(s)))
                $errorcase
            end
        end)
